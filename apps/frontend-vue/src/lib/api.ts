...
/**
 * Ported from the RN app's `lib/api.ts`. The interceptor logic — including the
 * single-flight refresh — is unchanged; only three things differ:
 *   - `process.env.EXPO_PUBLIC_API_URL` -> `import.meta.env.VITE_API_URL`
 *   - `useStore.getState()`             -> `useAuthStore()`
 *   - expo-router's `router.replace`    -> vue-router's `router.replace`
 */
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (status === 401 && originalRequest && !originalRequest._retry) {
      ...
      // the token storage) and send the user back to the entry screen.
      useAuthStore().logout()
      void router.replace('/')
    }
